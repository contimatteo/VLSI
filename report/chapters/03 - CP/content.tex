...
\begin{align}
  ...
  \text{c\_eq\_dim\_sym}          \leftarrow & \ \bigwedge_{(c_1, c_2) \in CC} (w_{c_1} == w_{c_2} \land h_{c_1} == h_{c_2}) \rightarrow lex\_lesseq([x_{c_1}, y_{c_1}], [x_{c_2}, y_{c_2}])                     \\
                                             & \nonumber                                                                                                                                                         \\
  \text{vc\_eq\_dim\_sym}         \leftarrow & \ \bigwedge_{(c, vc) \in C \times VC} \hspace{0.3cm} (\text{vc\_valid}(vc) \land w_{c_1} == w_{c_2} \land h_{c_1} == h_{c_2}) \rightarrow               \nonumber \\
                                             & \hspace{2.2cm} \rightarrow lex\_lesseq([x_{c_1}, y_{c_1}], [x_{c_2}, y_{c_2}])                                                                                    \\
                                             & \nonumber                                                                                                                                                         \\
...
The case we simplified is the one figured in \ref{fig:vc_specular_in}: instead of applying
