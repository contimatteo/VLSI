...
All the experiments for the CP technology have been executed on a machine with the following hardware:
...
The results obtained for \textit{rotation} models is quite different from before: we still have an increasing solve 
...
is highly limited and random search was not even implemented yet, but evidently rotating all circuits makes the instances
much more complicated.
After checking the solutions and seeing that most of the \textit{makespan} values (in \textit{base} models) match with our 
definition of \textit{min_makespan}, we may hazard that the dimensions of the circuits have been designed specifically for 
